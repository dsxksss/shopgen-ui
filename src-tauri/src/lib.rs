...
pub struct WorkflowPayload {
    ...
    inspectors: std::collections::BTreeMap<String, WorkflowInspector>,
}
...
fn build_tasks(plan: &OperationPlan) -> Vec<DashboardTask> {
    plan.agent_assignments.iter().enumerate().map(|(index, assignment)| {
        ...
        DashboardTask {
            ...
            total_steps,
            date: chrono::DateTime::parse_from_rfc3339(&plan.generated_at)
                .map(|dt| dt.format("%-d %b %Y").to_string())
                .unwrap_or_else(|_| plan.generated_at.clone()),
...
fn build_workflow(plan: &OperationPlan) -> WorkflowPayload {
    let mut nodes = vec![WorkflowNodeItem {
        id: "trigger".into(),
        kind: "trigger".into(),
        title: plan.scenario.clone(),
        subtitle: None,
    ...
    let mut inspectors = std::collections::BTreeMap::new();
    ...
    for (index, stage) in plan.workflow_stages.iter().enumerate() {
        ...
        nodes.push(WorkflowNodeItem {
            id: node_id.clone(),
            kind: "agent".into(),
            title: stage.name.clone(),
            subtitle: Some(format!("{} Agent", agent_name(&agent_id))),
            description: Some(stage.action.clone()),
            agent_id: Some(agent_id.clone()),
            status: Some(status_to_task_status(&stage.status).into()),
            x: 270 + (index as i32) * 280,
            y: 110 + ((index % 2) as i32) * 150,
        });

        edges.push(WorkflowEdgeItem {
        ...
        inspectors.insert(node_id.clone(), WorkflowInspector {
            ...
            inputs: vec![
                InspectorInput { label: "阶段目标".into(), value: stage.goal.clone() },
                InspectorInput { label: "负责人".into(), value: agent_name(&agent_id).into() },
                InspectorInput { label: "商家诉求".into(), value: plan.merchant_intent.clone() },
            ],
            tools: vec![
                InspectorTool { label: "任务拆解".into(), progress: if stage.status == "done" { 100 } else if stage.status == "active" { 72 } else { 0 }, active: stage.status == "active" },
                InspectorTool { label: "结果回填".into(), progress: if stage.status == "done" { 100 } else if stage.status == "active" { 45 } else { 0 }, active: false },
            ],
        });
    ...
    if !plan.risks.is_empty() {
        ...
        nodes.push(WorkflowNodeItem {
            ...
            kind: "condition".into(),
            title: "风险校验".into(),
            subtitle: Some("店长 Agent".into()),
            description: Some("核查执行风险、库存、利润与售后依赖项".into()),
            agent_id: Some("manager".into()),
            status: Some("in-progress".into()),
            x: 230 + (plan.workflow_stages.len() as i32) * 280,
            y: 240,
        });
        edges.push(WorkflowEdgeItem {
            id: format!("edge-{}-{}", previous_id, condition_id),
            source: previous_id,
        ...
        inspectors.insert(condition_id.clone(), WorkflowInspector {
            title: "风险校验".into(),
            agent_name: "店长 Agent".into(),
            status: "正在检查高风险项".into(),
            status_subtitle: format!("{} 条风险待关注", plan.risks.len()),
            status_tone: "in-progress".into(),
            inputs: plan.risks.iter().take(3).enumerate().map(|(index, risk)| InspectorInput { label: format!("风险 {}", index + 1), value: risk.clone() }).collect(),
            tools: vec![InspectorTool { label: "策略审查".into(), progress: 80, active: true }, InspectorTool { label: "执行确认".into(), progress: 30, active: false }],
        });
    }
...
fn build_wizard(prompt: &str, plan: &OperationPlan) -> WizardPayload {
    WizardPayload {
        ...
        breakdown: plan.workflow_stages.iter().take(3).map(|stage| stage.goal.clone()).collect(),
        team_agent_ids: plan.agent_assignments.iter().take(4).map(|assignment| normalize_agent_id(&assignment.agent_id)).collect(),
        team_objectives: plan.agent_assignments.iter().take(4).map(|assignment| WizardTeamObjective {
            agent_id: normalize_agent_id(&assignment.agent_id),
            agent_name: assignment.agent_name.clone(),
            objective: assignment.objective.clone(),
        }).collect(),
        ready_summary: plan.summary.clone(),
    }
}

fn build_workspace_view(prompt: String, plan: OperationPlan) -> WorkspaceView {
    let tasks = build_tasks(&plan);
    let workflow = build_workflow(&plan);
    let wizard = build_wizard(&prompt, &plan);
    WorkspaceView { plan, tasks, workflow, wizard }
}
...
fn get_runtime_status() -> RuntimeStatus {
    ...
    RuntimeStatus {
        ...
        base_url,
        model,
        ready_message: if configured { "Rust 后端已接入真实 AI 服务，可以直接生成运营方案。".into() } else { "尚未检测到 API Key，请先配置 ANTHROPIC_API_KEY。".into() },
    }
}

#[tauri::command]
async fn generate_workspace_view(prompt: String, scenario: String) -> Result<WorkspaceView, ApiError> {
    let plan = request_plan(prompt.clone(), scenario).await?;
    Ok(build_workspace_view(prompt, plan))
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![get_agent_catalog, get_runtime_status, generate_workspace_view])
